...
from ultralytics_mhaf.nn.modules import (
    AIFI,
    C1,
    C2,
    C3,
...
from ultralytics_mhaf.utils import DEFAULT_CFG_DICT, DEFAULT_CFG_KEYS, LOGGER, colorstr, emojis, yaml_load
from ultralytics_mhaf.utils.checks import check_requirements, check_suffix, check_yaml
from ultralytics_mhaf.utils.loss import v8ClassificationLoss, v8DetectionLoss, v8OBBLoss, v8PoseLoss, v8SegmentationLoss, v10DetectLoss, v10DetectATSSLoss, v10DetectLoss_aux
from ultralytics_mhaf.utils.plotting import feature_visualization
from ultralytics_mhaf.utils.torch_utils import (
    fuse_conv_and_bn,
    fuse_deconv_and_bn,
    initialize_weights,
    intersect_dicts,
...
class BaseModel(nn.Module):
    """The BaseModel class serves as a base class for all the models in the ultralytics_mhaf YOLO family."""
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def init_criterion(self):
        ...
        from ultralytics_mhaf.models.utils.loss import RTDETRDetectionLoss
...
def torch_safe_load(weight):
    ...
    try:
        with temporary_modules(
            {
                "ultralytics_mhaf.yolo.utils": "ultralytics_mhaf.utils",
                "ultralytics_mhaf.yolo.v8": "ultralytics_mhaf.models.yolo",
                "ultralytics_mhaf.yolo.data": "ultralytics_mhaf.data",
            }
        ...
        if e.name == "models":
            raise TypeError(
                emojis(
                    f"ERROR ❌️ {weight} appears to be an ultralytics_mhaf YOLOv5 model originally trained "
                    f"with https://github.com/ultralytics_mhaf/yolov5.\nThis model is NOT forwards compatible with "
                    f"YOLOv8 at https://github.com/ultralytics_mhaf/ultralytics_mhaf."
                    f"\nRecommend fixes are to train a new model using the latest 'ultralytics_mhaf' package or to "
                    f"run a command with an official YOLOv8 model, i.e. 'yolo predict model=yolov8n.pt'"
                )
            ) from e
        LOGGER.warning(
            f"WARNING ⚠️ {weight} appears to require '{e.name}', which is not in ultralytics_mhaf requirements."
...
def yaml_model_load(path):
    ...
    if path.stem in (f"yolov{d}{x}6" for x in "nsmlx" for d in (5, 8)):
        ...
        LOGGER.warning(f"WARNING ⚠️ ultralytics_mhaf YOLO P6 models now use -p6 suffix. Renaming {path.stem} to {new_stem}.")
