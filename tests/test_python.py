...
from ultralytics_mhaf import RTDETR, YOLO
from ultralytics_mhaf.cfg import TASK2DATA
from ultralytics_mhaf.data.build import load_inference_source
from ultralytics_mhaf.utils import (
    ASSETS,
    DEFAULT_CFG,
    DEFAULT_CFG_PATH,
    LINUX,
...
from ultralytics_mhaf.utils.torch_utils import TORCH_1_9, TORCH_1_13
...
def test_model_profile():
    ...
    from ultralytics_mhaf.nn.tasks import DetectionModel
...
def test_predict_img():
    ...
    batch = [
        str(SOURCE),  # filename
        Path(SOURCE),  # Path
        "https://ultralytics_mhaf.com/images/zidane.jpg" if ONLINE else SOURCE,  # URI
...
def test_track_stream():
    ...
    """
    video_url = "https://ultralytics_mhaf.com/assets/decelera_portrait_min.mov"
    model = YOLO(MODEL)
...
@pytest.mark.skipif(not ONLINE, reason="environment is offline")
def test_data_utils():
    """Test utility functions in ultralytics_mhaf/data/utils.py."""
    from ultralytics_mhaf.data.utils import HUBDatasetStats, autosplit
    from ultralytics_mhaf.utils.downloads import zip_directory

    # from ultralytics_mhaf.utils.files import WorkingDirectory
    # with WorkingDirectory(ROOT.parent / 'tests'):

    for task in "detect", "segment", "pose", "classify":
        file = Path(TASK2DATA[task]).with_suffix(".zip")  # i.e. coco8.zip
        download(f"https://github.com/ultralytics_mhaf/hub/raw/main/example_datasets/{file}", unzip=False, dir=TMP)
...
def test_data_converter():
    ...
    from ultralytics_mhaf.data.converter import coco80_to_coco91_class, convert_coco

    file = "instances_val2017.json"
    download(f"https://github.com/ultralytics_mhaf/yolov5/releases/download/v1.0/{file}", dir=TMP)
...
def test_data_annotator():
    ...
    from ultralytics_mhaf.data.annotator import auto_annotate
...
def test_events():
    ...
    from ultralytics_mhaf.hub.utils import Events
...
def test_cfg_init():
    ...
    from ultralytics_mhaf.cfg import check_dict_alignment, copy_default_cfg, smart_value
...
def test_utils_init():
    ...
    from ultralytics_mhaf.utils import get_git_branch, get_git_origin_url, get_ubuntu_version, is_github_action_running
...
def test_utils_checks():
    ...
    checks.check_imshow()
    checks.check_version("ultralytics_mhaf", "8.0.0")
    checks.print_args()
...
def test_utils_benchmarks():
    ...
    from ultralytics_mhaf.utils.benchmarks import ProfileModels
...
def test_utils_torchutils():
    ...
    from ultralytics_mhaf.nn.modules.conv import Conv
    from ultralytics_mhaf.utils.torch_utils import get_flops_with_torch_profiler, profile, time_sync
...
def test_utils_downloads():
    ...
    from ultralytics_mhaf.utils.downloads import get_google_drive_file_info
...
def test_utils_ops():
    ...
    from ultralytics_mhaf.utils.ops import (
        ltwh2xywh,
        ltwh2xyxy,
        make_divisible,
        xywh2ltwh,
...
def test_utils_files():
    ...
    from ultralytics_mhaf.utils.files import file_age, file_date, get_latest_run, spaces_in_path
...
def test_utils_patches_torch_save():
    ...
    from ultralytics_mhaf.utils.patches import torch_save

    mock = MagicMock(side_effect=RuntimeError)

    with patch("ultralytics_mhaf.utils.patches._torch_save", new=mock):
...
def test_nn_modules_conv():
    ...
    from ultralytics_mhaf.nn.modules.conv import CBAM, Conv2, ConvTranspose, DWConvTranspose2d, Focus
...
def test_nn_modules_block():
    ...
    from ultralytics_mhaf.nn.modules.block import C1, C3TR, BottleneckCSP, C3Ghost, C3x
...
@pytest.mark.skipif(not ONLINE, reason="environment is offline")
def test_hub():
    """Test ultralytics_mhaf HUB functionalities."""
    from ultralytics_mhaf.hub import export_fmts_hub, logout
    from ultralytics_mhaf.hub.utils import smart_request
...
def test_classify_transforms_train(image, auto_augment, erasing, force_color_jitter):
    ...
    from ultralytics_mhaf.data.augment import classify_augmentations
